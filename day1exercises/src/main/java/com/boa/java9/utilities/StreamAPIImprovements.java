...
public class StreamAPIImprovements {
	...
	//check it with object
	 ...
	 List<Customer> customers= createCustomers().stream().sorted((c1,c2)->c1.getDob().compareTo(c2.getDob()))
	 ...
	 //predicate not
	 createCustomers().stream().map(c->c.getCustomerId())
	            .filter(Predicate.not( StreamAPIImprovements::isEven))
